...
using EFCustomLogonWin.Module;
using EFCustomLogonWin.Module.BusinessObjects;
using System.Data.Common;
using EFCustomLogon.Module.BusinessObjects;

namespace EFCustomLogonWin.Win;

// For more typical usage scenarios, be sure to check out https://docs.devexpress.com/eXpressAppFramework/DevExpress.ExpressApp.Win.WinApplication._members
public class EFCustomLogonWinWindowsFormsApplication : WinApplication {
    public EFCustomLogonWinWindowsFormsApplication() {
		SplashScreen = new DXSplashScreen(typeof(XafSplashScreen), new DefaultOverlayFormOptions());
        ApplicationName = "EFCustomLogonWin";
        CheckCompatibilityType = DevExpress.ExpressApp.CheckCompatibilityType.DatabaseSchema;
        UseOldTemplates = false;
        DatabaseVersionMismatch += EFCustomLogonWinWindowsFormsApplication_DatabaseVersionMismatch;
        CustomizeLanguagesList += EFCustomLogonWinWindowsFormsApplication_CustomizeLanguagesList;
    ...
    private void EFCustomLogonWinWindowsFormsApplication_CustomizeLanguagesList(object sender, CustomizeLanguagesListEventArgs e) {
        string userLanguageName = System.Threading.Thread.CurrentThread.CurrentUICulture.Name;
        if(userLanguageName != "en-US" && e.Languages.IndexOf(userLanguageName) == -1) {
            e.Languages.Add(userLanguageName);
        }
    }
    private void EFCustomLogonWinWindowsFormsApplication_DatabaseVersionMismatch(object sender, DevExpress.ExpressApp.DatabaseVersionMismatchEventArgs e) {
#if EASYTEST
        e.Updater.Update();
        e.Handled = true;
#else
